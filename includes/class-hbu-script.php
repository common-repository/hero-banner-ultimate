...
class Hbu_Script {
	...
	function hbu_front_style() {
		...
		wp_register_style( 'hbu-public-style', HBU_URL.'assets/css/hbu-public-style.min.css', array(), HBU_VERSION );
	...
	function hbu_front_script() {
		...
		wp_register_script( 'hbu-video-script', HBU_URL.'assets/js/hbu-ultimate-bg.min.js', array('jquery'), HBU_VERSION, true );
		wp_register_script( 'hbu-public-script', HBU_URL.'assets/js/hbu-public-script.js', array('jquery'), HBU_VERSION, true );
	...
	function hbu_admin_script( $hook ) {
		...
		if( in_array($typenow, $pages_arr) ) {
			...
			wp_localize_script( 'hbu-admin-script', 'HbuproAdmin', array(
																		'new_ui'	=> $new_ui,
																		'sry_msg'	=> esc_html__('Sorry, One entry should be there.', 'hero-banner-ultimate'),
			));
		}
